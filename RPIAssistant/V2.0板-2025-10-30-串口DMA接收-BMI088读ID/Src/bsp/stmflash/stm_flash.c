...
void STMFLASH_Write ( uint32_t WriteAddr, uint16_t * pBuffer, uint16_t NumToWrite )	
{
	...
	while(1) 
	{	
		STMFLASH_Read_16(secpos*STM_SECTOR_SIZE+FLASH_BASE,STMFLASH_BUF,STM_SECTOR_SIZE/2);//读出整个扇区的内容
...
uint8_t STMFLASH_Write32(uint32_t WriteAddr,uint32_t *pBuffer,uint32_t NumToWrite)	
{ 
...
uint8_t STMFLASH_Write32_and_Erase(uint32_t WriteAddr,uint32_t *pBuffer,uint32_t NumToWrite)	
{ 
...
/**
  ...
  * 说    明：无
  */
void STMFLASH_Read_16 ( uint32_t ReadAddr, uint16_t *pBuffer, uint16_t NumToRead )   	
{
	uint16_t i;
...
/**
  ...
  * 说    明：无
  */
void STMFLASH_Read_32 ( uint32_t ReadAddr, uint32_t *pBuffer, uint32_t NumToRead )   	
{
	uint16_t i;
