...
class ROSMaster(object):
    ...
    # def set_motor(self, speed_1, speed_2, speed_3, speed_4):
        ...
        #     pass
    def set_motor(self, speed_1, speed_2, speed_3, speed_4):
        # 控制电机PWM脉冲，从而控制速度（未使用编码器测速）。speed_X=[-1000, 1000]
        # Control PWM pulse of motor to control speed (speed measurement without encoder). speed_X=[-1000, 1000]
        try:
            # 限制输入范围并打包成16位有符号整数
            speed_1_bytes = bytearray(struct.pack('h', self.__limit_motor_value_10000(speed_1)))
            speed_2_bytes = bytearray(struct.pack('h', self.__limit_motor_value_10000(speed_2)))
            speed_3_bytes = bytearray(struct.pack('h', self.__limit_motor_value_10000(speed_3)))
            speed_4_bytes = bytearray(struct.pack('h', self.__limit_motor_value_10000(speed_4)))
    ...
    def __limit_motor_value_10000(self, value):
        # 限制范围到[-1000, 1000]
        if value > 10000:
            return 10000
        elif value < -10000:
